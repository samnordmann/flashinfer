...
__device__ void hmma_16_8_16_f32acc_bf16ab(float (&d_reg)[4], const bf16_t (&a_reg)[8],
                                           const bf16_t (&b_reg)[4], float const (&c_reg)[4]) {
  ...
  asm volatile(
      ...
      : "r"(a0), "r"(a1), "r"(a2), "r"(a3), "r"(b0), "r"(b1), "f"(d_reg[0]), "f"(d_reg[1]),
        "f"(d_reg[2]), "f"(d_reg[3]));
#endif
}
...
struct MmaComputer {
  ...
  __device__ MmaComputer(uint8_t* gmem_c_local_, uint8_t* gmem_scale_local_, bf16_t* smem_a_,
                         bf16_t* smem_b_, uint64_t* smem_barrier_, int warp_idx_, int gemm_n_,
                         float const* global_scale_)
      : gmem_c(gmem_c_local_),
        gmem_scale(gmem_scale_local_),
        smem_a(smem_a_),
        smem_b(smem_b_),
        smem_barrier(smem_barrier_),
...
#pragma unroll
      static_assert(tile_m == 16, "NVFP4 epilogue requires one scale block per CTA");
#pragma unroll
      for (int reg_idx = 0; reg_idx < final_acc_reg_cnt; reg_idx++) {
        int linear_idx = reg_idx * 32 + lane_idx;
        int m_idx = linear_idx % tile_m;
        int n_idx = linear_idx / tile_m;
        float value = __bfloat162float(__float2bfloat16(acc_final[reg_idx]));
        float block_amax = fabsf(value);
        block_amax = fmaxf(block_amax, __shfl_xor_sync(0xffffffff, block_amax, 8, 16));
        block_amax = fmaxf(block_amax, __shfl_xor_sync(0xffffffff, block_amax, 4, 16));
        block_amax = fmaxf(block_amax, __shfl_xor_sync(0xffffffff, block_amax, 2, 16));
        block_amax = fmaxf(block_amax, __shfl_xor_sync(0xffffffff, block_amax, 1, 16));

        float global_scale_value = *global_scale;
        float sf_value = global_scale_value * (block_amax / 6.0f);
        __nv_fp8_e4m3 fp8_scale(sf_value);
        sf_value = static_cast<float>(fp8_scale);
        float output_scale = sf_value != 0.0f ? global_scale_value / sf_value : 0.0f;
        float quant_value = value * output_scale;
        float peer_value = __shfl_xor_sync(0xffffffff, quant_value, 1, 16);

        if (n_idx < gemm_n) {
          if (m_idx == 0) {
            gmem_scale[n_idx * (gemm_m / 16)] = fp8_scale.__x;
          }
          if ((m_idx & 1) == 0) {
            __nv_fp4x2_storage_t packed = __nv_cvt_float2_to_fp4x2(
                make_float2(quant_value, peer_value), __NV_E2M1, cudaRoundNearest);
            gmem_c[n_idx * (gemm_m / 2) + m_idx / 2] = packed;
          }
        }
      }
    }
...
__global__ __launch_bounds__(256,
                             1) void fused_a_gemm_kernel(uint8_t* output, uint8_t* output_scale,
                                                         bf16_t const* mat_a, bf16_t const* mat_b,
                                                         int gemm_n, float const* global_scale) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 900
  ...
  int valid_tokens = min(tile_n, gemm_n - cta_n_idx);
  bf16_t const* gmem_a_local = mat_a + cta_m_idx * gemm_k;
  bf16_t const* gmem_b_local = mat_b + cta_n_idx * gemm_k;
  uint8_t* gmem_c_local = output + cta_n_idx * (gemm_m / 2) + cta_m_idx / 2;
  uint8_t* gmem_scale_local = output_scale + cta_n_idx * (gemm_m / 16) + cta_m_idx / 16;
  ...
  if (warp_idx < 2) {
    ...
    GmemLoaderB<gemm_k, tile_n, tile_k, stage_cnt> b_loader(gmem_b_local, smem_b, smem_barrier,
                                                            valid_tokens);
    b_loader.prepare();
    b_loader.issue_mainloop();
  } else {
    MmaComputer<gemm_m, gemm_k, tile_m, tile_n, tile_k, stage_cnt> mma_computer(
        gmem_c_local, gmem_scale_local, smem_a, smem_b, smem_barrier, warp_idx, valid_tokens,
        global_scale);
...
template <int kHdIn, int kHdOut, int kTileN, bool kUsePDL>
cudaError_t invokeBf16GemmNvfp4(uint8_t* output, uint8_t* output_scale, bf16_t const* input,
                                bf16_t const* weight, int num_tokens, float const* global_scale,
                                cudaStream_t stream) {
  ...
  constexpr int smem_bytes =
      ((tile_m * 2 + tile_n * 2) * tile_k * stage_cnt + barrier_bytes + 1023) / 1024 * 1024;
  ...
  config.gridDim = dim3(cta_m_cnt, cta_n_cnt, 1);
  config.blockDim = dim3(256);
  config.dynamicSmemBytes = smem_bytes;
  config.stream = stream;
  cudaLaunchAttribute attrs[1];
  attrs[0].id = cudaLaunchAttributeProgrammaticStreamSerialization;
  attrs[0].val.programmaticStreamSerializationAllowed = kUsePDL;
  config.attrs = attrs;
  config.numAttrs = 1;
  return cudaLaunchKernelEx(&config, kernel, output, output_scale, weight, input, gemm_n,
                            global_scale);
}
